...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while((int)get_current_batch(net) < net->max_batches){
        if(l.random && count++%10 == 0){
            ...
            if ((int)get_current_batch(net)+200 > net->max_batches) dim = 608;
...
void print_imagenet_detections(FILE *fp, int id, detection *dets, int total, int classes, int w, int h)
{
    ...
    for(i = 0; i < total; ++i){
        ...
        for(j = 0; j < classes; ++j){
            int class_val = j;
            if (dets[i].prob[class_val]) fprintf(fp, "%d %d %f %f %f %f %f\n", id, j+1, dets[i].prob[class_val],
                    xmin, ymin, xmax, ymax);
        }
    }
}
...
void run_detector(int argc, char **argv)
{
    ...
    //int class = find_int_arg(argc, argv, (char *)"-class", 0);

    char *datacfg = argv[3];
    char *cfg = argv[4];
    char *weights = (argc > 5) ? argv[5] : 0;
    char *filename = (argc > 6) ? argv[6]: 0;
    if(0==strcmp(argv[2], (char *)"test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, outfile, fullscreen);
    else if(0==strcmp(argv[2], (char *)"train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear);
    else if(0==strcmp(argv[2], (char *)"valid")) validate_detector(datacfg, cfg, weights, outfile);
    else if(0==strcmp(argv[2], (char *)"valid2")) validate_detector_flip(datacfg, cfg, weights, outfile);
    else if(0==strcmp(argv[2], (char *)"recall")) validate_detector_recall(cfg, weights);
    else if(0==strcmp(argv[2], (char *)"demo")) {
        list *options = read_data_cfg(datacfg);
    ...
    //else if(0==strcmp(argv[2], (char *)"extract")) extract_detector(datacfg, cfg, weights, cam_index, filename, class, thresh, frame_skip);
    //else if(0==strcmp(argv[2], (char *)"censor")) censor_detector(datacfg, cfg, weights, cam_index, filename, class, thresh, frame_skip);
}
