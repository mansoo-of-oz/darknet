...
void test_cifar_multi(char *filename, char *weightfile)
{
    ...
    for(i = 0; i < test.X.rows; ++i){
        ...
        int class_val = max_index(test.y.vals[i], 10);
        if(index == class_val) avg_acc += 1;
        free_image(im);
        printf("%4d: %.2f%%\n", i, 100.*avg_acc/(i+1));
    }
}
...
void extract_cifar()
{
    ...
    for(i = 0; i < train.X.rows; ++i){
        ...
        int class_val = max_index(train.y.vals[i], 10);
        char buff[256];
        sprintf(buff, (char *)"data/cifar/train/%d_%s",i,labels[class_val]);
    ...
    for(i = 0; i < test.X.rows; ++i){
        ...
        int class_val = max_index(test.y.vals[i], 10);
        char buff[256];
        sprintf(buff, (char *)"data/cifar/test/%d_%s",i,labels[class_val]);
        save_image_png(im, buff);
    }
}
...
void run_cifar(int argc, char **argv)
{
    ...
    if(0==strcmp(argv[2], (char *)"train")) train_cifar(cfg, weights);
    else if(0==strcmp(argv[2], (char *)"extract")) extract_cifar();
    else if(0==strcmp(argv[2], (char *)"distill")) train_cifar_distill(cfg, weights);
    else if(0==strcmp(argv[2], (char *)"test")) test_cifar(cfg, weights);
    else if(0==strcmp(argv[2], (char *)"multi")) test_cifar_multi(cfg, weights);
    else if(0==strcmp(argv[2], (char *)"csv")) test_cifar_csv(cfg, weights);
    else if(0==strcmp(argv[2], (char *)"csvtrain")) test_cifar_csvtrain(cfg, weights);
    else if(0==strcmp(argv[2], (char *)"eval")) eval_cifar_csv();
}
